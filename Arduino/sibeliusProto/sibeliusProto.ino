//log
//most of the code(especially any odd ones) in/for loop() is written to optimise loop speed
...
void loop() {
  ...
  /* Relay setup */
  static bool check = false;  //exists to link into the following while loop
  static bool run = false;    //exists to link from while loop into relay loop
  ...
  static int pinConfig[3];       //solenoid, pump, button    
  ...
  if (digitalRead(w_button) == HIGH){
    ...
    runtimeWater = interface.obtainConfig(runtimeConfig, overrideTrigger); 
  ...
  static unsigned long timeElapsed; //has it's own variable, since millis() changes b/w lines of codes
  if (check == true){
    while (digitalRead(w_button) == HIGH ||digitalRead(c_button) == HIGH){} //catches button holds

    if (runtimeConfig == water){
      timeElapsed = millis(); 
      runtime = timeElapsed + runtimeWater; 
    }
    else{
      timeElapsed = millis();
  ...
  static bool enablePumps; //exists to add code that runs once when enabling pumps
  ...
  while (run == true){ 
    ...
    //this stop code is before the times get updated below so if runtime =0, duration will =0
    if (digitalRead(pinConfig[2]) == HIGH || timeElapsed >= runtime){
      interface.runFinish(runtimeConfig, (timeElapsed-timeStart)); 
